"""Admin API — manual page save for visual builder."""
...
@require_POST
def page_save_view(request, post_id: int):
    ...
    if payload is None:
        log_page_save_invalid_json(post_id=post.pk, user_id=_user_id(request))
    ...
    try:
        with transaction.atomic():
            locked_post = BlogPost.objects.select_for_update().get(pk=post.pk)
            ...
            if result.changed:
                ...
                locked_post.save(update_fields=update_fields)
            else:
                locked_post.save(update_fields=update_fields)
        ...
        log_page_save_unexpected_error(post_id=post.pk, user_id=_user_id(request), exc=exc)
        return JsonResponse(
            {
                "ok": False,
                "error": "server_error",
    ...
    locked_post.refresh_from_db(fields=["updated_at", "body_plaintext", "page_version"])

    return JsonResponse(
        {
            "ok": True,
            "changed": result.changed,
            "page_version": locked_post.page_version,
            "saved_at": locked_post.updated_at.isoformat(),
            "word_count": word_count(locked_post.body_plaintext),
        }
    )
